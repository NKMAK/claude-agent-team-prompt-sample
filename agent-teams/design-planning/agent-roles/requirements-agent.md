...
- **ユーザーから提供された情報のみを要件に記載する**
- 提案内容や要件の詳細が不明な場合、自ら補完・推測せず、必ずユーザーに質問する
- 設計フェーズ（Planning/Architect作業中）においても、疑問点が生じたらAskUserQuestionでユーザーに確認し続ける
...
6. **ユーザーの言葉を要件にする**: AIが「よさそう」と判断した内容を勝手に要件に追加しない。不明点は常に質問する
7. **設計中も質問を継続する**: 初回ヒアリング完了後も、Planning/Architectから疑問が来た場合はユーザーに確認し、設計完了まで待機してサポートする
